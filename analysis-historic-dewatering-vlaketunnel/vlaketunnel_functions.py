...
def create_model(
    ...
    """
    Create a timflow model for Vlaketunnel case
...
def plot_model_input(ml):
    ...
    # plot channel
    channel = next(iter(ml.aq.inhomdict.values()))
    plt.plot([xmin, xchannel], [channel.hstar] * 2, color="blue")
    plt.annotate(
        f"h_ch={channel.hstar:0.1f}", (xchannel, channel.hstar), ha="right", va="bottom"
    )
    plt.annotate(
        f"c_ch={channel.c[0]:0.1f}",
...
def plot_model_results(ml, df_dh):
    """
    Plot results of timflow model of Vlaketunnel case
